...
function EmailVerification() {
  ...
  const handleResendEmail = async () => {
    setLoading(true);
    try {
      // Reload user to ensure sync with Firebase
      if (currentUser) {
  ...
  const handleCheckVerification = async () => {
    ...
    try {
      ...
      if (isVerified) {
        ...
        notifyInfo("Email not verified yet. Please check your inbox AND spam/junk folder and click the verification link.");
      }
    } catch (error) {
